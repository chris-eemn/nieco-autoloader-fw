#!/usr/bin/sh
# special sh script for running in an MSYS2 (gitbash) environment
...
MSYS_NO_PATH_CONV=1 MSYS2_ARG_CONV_EXCL="*" docker run --workdir=/project --rm -v "$REPO_ROOT:/project" "$IMAGE" /bin/bash -c ". create_release/gen_build.bash"
