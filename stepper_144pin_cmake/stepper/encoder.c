...
struct encoder_s {
  hal_tim_handle_t *htim;        /* Timer handle in encoder mode (caller-owned)      */
...
int32_t encoder_get_count(encoder_t *enc) {
  ...
  uint32_t raw = HAL_TIM_GetCounter(enc->htim);
   ...
   * free-running counter. The int16_t cast yields the signed displacement. */
   // this trick only works if the timer ARR 0xffff
...
void encoder_zero(encoder_t *enc) {
  ...
  HAL_TIM_SetCounter(enc->htim, 0x8000U);
