...
UINT _ux_hcd_stm32_request_control_transfer(UX_HCD_STM32 *hcd_stm32, UX_TRANSFER *transfer_request)
{
  ...
  case UX_HCD_STM32_ED_STATUS_CONTROL_DATA_OUT | UX_HCD_STM32_ED_STATUS_TRANSFER_DONE:
    ...
    if (ed -> ux_stm32_ed_dir)
    {
      ...
      transfer_request -> ux_transfer_request_actual_length =
                      HAL_HCD_GetChannelTransferCount(hcd_stm32 -> hcd_handle, ed -> ux_stm32_ed_channel);
    }
    else
    {
  ...
  /* LOCAL PATCH (not upstream ST/Eclipse ThreadX -- see CLAUDE.md "Local patches to vendored code").
   * ux_stm32_ed_data aliases ux_stm32_ed_setup during the SETUP stage (see
   * _ux_hcd_stm32_request_control_setup() below), so it must be cleared here as well. Without this,
   * a control transfer with no data stage (UX_SET_ADDRESS, UX_SET_CONFIGURATION) leaves ed_data
   * dangling at this freed block: _ux_hcd_stm32_request_control_status() never reassigns ed_data,
   * and the status-stage completion runs _ux_hcd_stm32_request_trans_finish() from the USB ISR
   * (ux_hcd_stm32_callback.c), where none of its guards catch the stale alias -- ed_data != UX_NULL,
   * ed_data != transfer_request -> ux_transfer_request_data_pointer (UX_NULL for a zero-length
   * request), and ed_data != ed_setup (just nulled above) -- so it double-frees it from interrupt
   * context. That surfaces as a UX_MUTEX_ERROR/UX_MUTEX_ERROR/UX_MEMORY_CORRUPTED burst per
   * enumeration (the FreeRTOS port's mutex refuses to run inside an ISR, and the allocator then
   * rejects the double free). */
                                                       ...
                                                       UX_HCD_STM32_ED *ed, UX_ENDPOINT *endpoint, UX_TRANSFER *transfer_request)
{
  ...
  *(setup_request + UX_SETUP_REQUEST) = transfer_request -> ux_transfer_request_function;
  ...
  case UX_HIGH_SPEED_DEVICE:
    ed -> ux_stm32_ed_speed = UX_HIGH_SPEED_DEVICE;
    break;
  case UX_LOW_SPEED_DEVICE:
    ed -> ux_stm32_ed_speed = UX_LOW_SPEED_DEVICE;
    break;
  default:
    ed -> ux_stm32_ed_speed = UX_FULL_SPEED_DEVICE;
    break;
  }

  /* Initialize the host channel for SETUP phase.  */
  ed -> ux_stm32_ed_dir = HAL_HCD_CH_OUT_DIR;

  ed->ux_stm32_ch_config.device_address = endpoint -> ux_endpoint_device -> ux_device_address;
  ed->ux_stm32_ch_config.device_speed = (hal_hcd_device_speed_t)ed -> ux_stm32_ed_speed;
  ed->ux_stm32_ch_config.ep_address = 0U;
  ed->ux_stm32_ch_config.ep_type = HAL_HCD_EP_TYPE_CTRL;
  ed->ux_stm32_ch_config.ep_mps = endpoint -> ux_endpoint_descriptor.wMaxPacketSize;

  HAL_HCD_SetConfigChannel(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, &ed->ux_stm32_ch_config);

#if defined (USE_HAL_HCD_USB_HUB_HS_SPLIT) && (USE_HAL_HCD_USB_HUB_HS_SPLIT == 1U) && (UX_MAX_DEVICES > 1)
  /* Check if device connected to hub  */
  if (endpoint->ux_endpoint_device->ux_device_parent != NULL)
  {
    HAL_HCD_SetChannelHubInfo(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel,
                              endpoint->ux_endpoint_device->ux_device_parent->ux_device_address,
                              endpoint->ux_endpoint_device->ux_device_port_location);
  }
#endif /* defined (USE_HAL_HCD_USB_HUB_HS_SPLIT) && (USE_HAL_HCD_USB_HUB_HS_SPLIT == 1U) && (UX_MAX_DEVICES > 1) */

  /* Send the SETUP packet.  */
  ed->ux_stm32_ch_transfer_request.ch_dir = HAL_HCD_CH_OUT_DIR;
  ed->ux_stm32_ch_transfer_request.ep_type = HAL_HCD_EP_TYPE_CTRL;
  ed->ux_stm32_ch_transfer_request.transfer_length = 8U;
  ed->ux_stm32_ch_transfer_request.p_buffer = setup_request;
  ed->ux_stm32_ch_transfer_request.token_type = USBH_PID_SETUP;
...
static inline VOID _ux_hcd_stm32_request_control_data(UX_HCD_STM32 *hcd_stm32,
                                                      UX_HCD_STM32_ED *ed, UX_ENDPOINT *endpoint, UX_TRANSFER *transfer_request)
{
  hal_hcd_dma_status_t      dma_status;
  ...
  if ((transfer_request -> ux_transfer_request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_IN)
  {
    ...
    ed -> ux_stm32_ed_dir = HAL_HCD_CH_IN_DIR;

    ed->ux_stm32_ch_config.device_address = endpoint->ux_endpoint_device->ux_device_address;
    ed->ux_stm32_ch_config.device_speed = (hal_hcd_device_speed_t)ed->ux_stm32_ed_speed;
    ed->ux_stm32_ch_config.ep_address = 0x80U;
    ed->ux_stm32_ch_config.ep_type = HAL_HCD_EP_TYPE_CTRL;
    ed->ux_stm32_ch_config.ep_mps = endpoint->ux_endpoint_descriptor.wMaxPacketSize;
  ...
  dma_status = (hal_hcd_dma_status_t)(hcd_stm32->hcd_handle->driver.core_get_dma_status((UINT)hcd_stm32->hcd_handle->instance));
  ...
  if ((ed -> ux_stm32_ed_dir == 0) &&
      (transfer_request -> ux_transfer_request_requested_length > endpoint -> ux_endpoint_descriptor.wMaxPacketSize) &&
        (dma_status == HAL_HCD_DMA_DISABLED))
  {
  ...
  ed->ux_stm32_ch_transfer_request.p_buffer = ed -> ux_stm32_ed_data;
                                                        ...
                                                        UX_HCD_STM32_ED *ed, UX_ENDPOINT *endpoint, UX_TRANSFER *transfer_request)
{
  ...
  ed -> ux_stm32_ed_dir = (hal_hcd_ch_direction_t)(!ed -> ux_stm32_ed_dir);

  ed->ux_stm32_ch_config.device_address = endpoint -> ux_endpoint_device -> ux_device_address;
  ed->ux_stm32_ch_config.device_speed = (hal_hcd_device_speed_t)ed -> ux_stm32_ed_speed;
  ed->ux_stm32_ch_config.ep_address = ed -> ux_stm32_ed_dir ? 0x80U : 0x00U;
  ed->ux_stm32_ch_config.ep_type = HAL_HCD_EP_TYPE_CTRL;
  ed->ux_stm32_ch_config.ep_mps = endpoint -> ux_endpoint_descriptor.wMaxPacketSize;
  ...
  ed->ux_stm32_ch_transfer_request.ch_dir = ed -> ux_stm32_ed_dir;
  ed->ux_stm32_ch_transfer_request.ep_type = HAL_HCD_EP_TYPE_CTRL;
  ed->ux_stm32_ch_transfer_request.transfer_length = 0U;
  ed->ux_stm32_ch_transfer_request.p_buffer = ed -> ux_stm32_ed_data;
  ed->ux_stm32_ch_transfer_request.token_type = USBH_PID_DATA;
  ed->ux_stm32_ch_transfer_request.do_ping = 0U;
